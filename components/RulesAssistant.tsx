...
export const RulesAssistant: React.FC<RulesAssistantProps> = ({ game, onSaveRules, onClose }) => {
  ...
  const handleGenerateCodex = async () => {
    ...
    setLoading(true);
    try {
        const resourceDesc = rules.resourceType === 'MANA_RAMP' ? "Accumulating Mana" : "Fixed Energy per Turn";
        
        // 1. Generate Text Rulebook
        const result = await generateRuleAssistance(game.name, `Reduce HP from ${rules.initialHealth}`, resourceDesc, rules.boardType);
        const newRules = { ...rules, fullText: result };
        setRules(newRules);
        
        // 2. Generate Visual Promo Cards with User Config (Using the Archetype slots!)
        const promos = await generatePromoCards(game.name, result, codexSlots);
                    ...
                    <div>
                         <div className="flex items-center justify-between mb-4">
                            ...
                            <button 
                                ...
                                disabled={loading}
                                className="text-[10px] font-bold text-slate-400 hover:text-white flex items-center gap-1 bg-white/5 px-3 py-1 rounded-full border border-white/10"
                            >
                                <i className={`fas fa-sync ${loading ? 'fa-spin' : ''}`}></i> {promoCards.length > 0 ? 'Regenerate' : 'Generate'}
                            </button>
                         </div>
                         ...
                         <div className="flex gap-4 overflow-x-auto pb-6 snap-x no-scrollbar">
                             {(loading || promoCards.length === 0) && (
                                 <div className="w-full h-64 flex flex-col items-center justify-center text-slate-500 border border-white/10 rounded-2xl bg-black/40 p-8 text-center">
                                     {loading ? (
                                         <span className="animate-pulse text-xs uppercase font-bold text-fuchsia-400">Writing Codex...</span>
                                     ) : (
                                         <>
                                            <i className="fas fa-book-open text-3xl mb-3 opacity-30"></i>
                                            <span className="text-xs uppercase font-bold">Select an Archetype & Generate</span>
                                         </>
                                     )}
                                 </div>
                             )}
                             ...
                             {promoCards.map((card, i) => (
                                 <div key={i} className="snap-center shrink-0 w-64 h-80 relative rounded-2xl overflow-hidden group shadow-xl border border-white/10 bg-black animate-slideUp" style={{ animationDelay: `${i * 100}ms` }}>
                                     ...
                                     <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10">
                                         <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                                            <i className={`fas ${card.icon} text-4xl text-fuchsia-400`}></i>
                                         </div>
                                         <h3 className="text-xl font-black text-white uppercase mb-4 leading-none">{card.title}</h3>
                                         <p className="text-xs text-slate-300 leading-relaxed font-medium">{card.description}</p>
                                     </div>
                         ...
                         {/* Explanatory Text */}
                         <p className="text-[10px] text-slate-500 mt-1">These cards will be displayed in the <strong>Guide</strong> tab to teach players your game's specific strategy.</p>
                    </div>
