...
export const GameTable: React.FC<GameTableProps> = ({ game, cards, decks }) => {
  ...
  // RULES SNAPSHOT (SAFETY MECHANISM)
  ...
  if (playState === 'LOBBY') {
       const activeBg = lobbyBg || game.boardTheme?.backgroundUrl;
       const selectedDeck = activeDecks.find(d => d.id === selectedDeckId);
       const isValid = selectedDeck ? isDeckValid(selectedDeck) : false;
       return (
          <div className="flex flex-col h-full bg-[#0f0b15] relative overflow-hidden">
               {/* Simplified Lobby Render for brevity */}
               <div className="absolute inset-0 z-0">
                    <img src={activeBg || 'https://www.transparenttextures.com/patterns/dark-matter.png'} className="w-full h-full object-cover opacity-30" />
               </div>
               
               {/* DECK SELECTOR (Simplified) */}
               {showDeckSelector && (
                  <div className="absolute inset-0 z-50 bg-[#0f0b15]/95 p-6 overflow-y-auto">
                       <button onClick={() => setShowDeckSelector(false)} className="text-white mb-4">Close</button>
                       <div className="grid grid-cols-2 gap-4">
                           {activeDecks.map(deck => (
                               <button key={deck.id} onClick={() => {setSelectedDeckId(deck.id); setShowDeckSelector(false)}} className="bg-slate-800 p-4 rounded text-white text-xs">{deck.name}</button>
                           ))}
                       </div>
                  </div>
               )}
               ...
               <div className="relative z-10 flex-1 flex flex-col items-center justify-center">
                    <h1 className="text-3xl font-black text-white italic uppercase mb-8">{game.name}</h1>
                    <button onClick={() => setShowDeckSelector(true)} className="bg-slate-800 border border-white/20 p-4 rounded-xl mb-8">
                        {selectedDeck ? selectedDeck.name : "Select Deck"}
                    </button>
                    <button onClick={handleStartSearch} disabled={!isValid} className="bg-fuchsia-600 px-8 py-4 rounded-full font-black text-white uppercase disabled:opacity-50">
                        VS CPU
                    </button>
                    {/* Context Switch */}
                     <button onClick={() => { setUseGlobalDecks(!useGlobalDecks); setSelectedDeckId(''); }} className="mt-4 text-xs text-cyan-400 font-bold uppercase border border-cyan-500/30 px-3 py-1 rounded-full">
                          {useGlobalDecks ? 'Global Context' : 'Local Context'}
                     </button>
               </div>
          </div>
       )
