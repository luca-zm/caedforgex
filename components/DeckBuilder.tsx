...
// 2. Auto Scaling Card (Now optimized for 4 cols)
const AutoScalingCard = ({ card, count, isSelected, onToggle, onInspect }: any) => {
    return (
        <div className="w-full relative group z-0 aspect-[240/340]">
            {/* Base Card */}
            <div className="absolute inset-0 transform scale-[0.95] group-hover:scale-100 transition-transform duration-200">
            ...
            <div 
                ...
                className={`absolute inset-0 rounded-[14px] cursor-pointer transition-all duration-200 z-10
                    ${isSelected ? 'ring-4 ring-fuchsia-500 ring-offset-2 ring-offset-black bg-fuchsia-500/10' : 'hover:bg-white/5'}
                `}
            ></div>

            {/* Count Badge (Supercell Style) */}
            {count > 0 && (
                <div className="absolute -top-2 -right-2 w-7 h-7 bg-gradient-to-br from-fuchsia-500 to-purple-700 rounded-lg border-2 border-white shadow-[0_4px_10px_rgba(0,0,0,0.5)] flex items-center justify-center z-20 animate-bounce-short">
                    <span className="text-white font-black text-xs drop-shadow-md">x{count}</span>
                </div>
            )}
            ...
            <button
                ...
                className="absolute bottom-1 right-1 w-6 h-6 rounded-full bg-black/60 text-white/70 hover:text-white flex items-center justify-center z-20 border border-white/10 backdrop-blur-sm"
            >
...
export const DeckBuilder: React.FC<DeckBuilderProps> = ({ 
  ...
  return (
    <div className="h-full flex flex-col bg-[#0f0b15] relative animate-fadeIn">
        ...
        <div className="shrink-0 bg-[#141218] border-b border-white/10 shadow-2xl z-30">
            {/* Info Bar */}
            <div className="px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <button onClick={() => onSelectDeck('')} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-white">
                    ...
                    <div>
                        ...
                        <div className={`text-[10px] font-bold uppercase ${status.color}`}>
                            {currentDeck.cardIds.length}/{game.rules?.constraints.maxCards || 40} • {status.label}
                        </div>
                    </div>
                </div>
                {/* Mana Curve Visual */}
            ...
            {/* Deck Tray (Horizontal Scroll of selected cards) */}
            <div className="bg-black/40 py-2">
                <div className="flex gap-2 px-4 overflow-x-auto no-scrollbar h-20 items-center">
                    {distinctDeckCards.length === 0 && (
                        <div className="text-[10px] text-slate-600 font-bold uppercase w-full text-center">Empty Deck - Tap cards below to add</div>
                    )}
                    {distinctDeckCards.map(({card, count}) => (
                         <div 
                            key={card.id} 
                            onClick={() => removeCardFromDeck(card.id)}
                            className="relative shrink-0 w-12 h-16 rounded bg-slate-800 border border-white/20 overflow-hidden cursor-pointer hover:border-red-500 transition-colors group"
                         >
                            <img src={card.imageUrl} className="w-full h-full object-cover opacity-80" alt="" />
                            {/* Count Badge */}
                            <div className="absolute bottom-0 right-0 bg-black/80 text-white text-[9px] font-black px-1.5 py-0.5 rounded-tl">x{count}</div>
                            {/* Remove Overlay */}
