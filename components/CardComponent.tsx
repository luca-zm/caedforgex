      ...
      >
        ...
        <div className={`absolute inset-0 w-full h-full ${staticMode ? '' : 'backface-hidden'}`}>
            ...
            <div className="absolute inset-0 rounded-[18px] overflow-hidden shadow-2xl border-[3px] border-[#1e1b2e] bg-[#151320] z-0">
                ...
                {/* MAIN IMAGE */}
                <div className="absolute top-9 left-2 right-2 h-[50%] bg-[#0a0a0a] rounded-lg overflow-hidden border-2 border-[#3f3b4a] shadow-inner group">
                    {card.imageUrl ? (
                        <img src={card.imageUrl} className={`w-full h-full object-cover ${!staticMode && 'transition-transform duration-700 group-hover:scale-110'}`} alt="Card Art" loading="lazy" />
                    ) : (
                    ...
                    {!staticMode && <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>}
                </div>
