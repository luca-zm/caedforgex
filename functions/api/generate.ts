...
export async function onRequest(context: any) {
    ...
    try {
        ...
        if (type === 'ui_background' || type === 'card_art' || type === 'world_icon' || type === 'board_art') {
            ...
            if (responseImg.generatedImages && responseImg.generatedImages.length > 0) {
                const imgBase64 = responseImg.generatedImages[0].image?.imageBytes;
                if (imgBase64) {
                    return new Response(JSON.stringify({ image: `data:image/png;base64,${imgBase64}` }), {
