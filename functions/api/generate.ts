...
export async function onRequest(context: any) {
    ...
    try {
        ...
        if (type === 'ui_background' || type === 'card_art' || type === 'world_icon' || type === 'board_art') {
            ...
            // BACK TO GEMINI-2.5-FLASH-IMAGE (Nano Banana) FOR FREE TIER
            // The Google Gen AI SDK for v1beta requires generateContent with responseModalities for this model
            ...
            // The imageBytes are now returned inside the response base64 or inlineData depending on the exact schema flavor
            // Usually, genai returns it in candidates[0].content.parts[0].inlineData.data
